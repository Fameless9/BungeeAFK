...
The language setting allows you to choose the language for the plugin's messages. You can set it to `en` for English or
`de` for German.  
...
Do not change this value unless you know what you are doing. This is the port used for communication between the proxy
and sub-servers.
...
This is the delay in seconds after which a warning message is sent to the player after their last activity. The warning
message  
...
- `PER_SERVER`: Notifications are sent only to players on the same server as the AFK player, except for the player who
  has gone AFK.
- `DISABLE`: No notifications are sent.

This option affects the following language keys: `notification.afk_broadcast`, `notification.return_broadcast`,
`notification.afk_kick_broadcast`, `notification.afk_disconnect_broadcast`.
...
Example: If set to `5`, the player must visit the same block at least `5` times before the movement pattern can be
analyzed.
